...
pthread_mutex_t page_mutex[THREAD_NUM] = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t buffer_cond = PTHREAD_COND_INITIALIZER;
pthread_cond_t page_cond[THREAD_NUM] = PTHREAD_COND_INITIALIZER;
int trx_id;
trx * transaction;
int work_done = 0;
//int fd;
...
int trx_begin(){
    transaction[trx_id].thread_id = trx_id;
    transaction[trx_id].thread = NULL;
    return trx_id++;
}
void * trx_db_find(int table_id, pagenum_t key, char* value, int trx_id){
    while(work_done < THREAD_NUM){
...
int multi_thread(int table_id, pagenum_t key, char* value)
{
    ...
    trx_id = 0;
    transaction = malloc(sizeof(trx) * THREAD_NUM);
    for(int i = 0; i < THREAD_NUM; i++){
        printf("thread id %d\n",trx_begin());
    }
    
    for(int i = 0; i < THREAD_NUM; i++){
    pthread_create(&transaction[i].thread, 0, trx_db_find(table_id, key, value, i), NULL);
    }
    
    for(int i = 0; i < THREAD_NUM; i++){
    pthread_join(transaction[i].thread, NULL);
    }
...
int db_find(int table_id, pagenum_t key, char * ret_val, int trx_id){
    ...
    pthread_mutex_lock(&page_mutex[table_id-1]);   // lock the page in buffer
    ...
    pin_plus(index);
    pthread_mutex_unlock(&page_mutex[table_id-1]);    // unlock the page in buffer
    find = Lock_acquire(table_id-1, key, trx_id, lock_mode);    // lock the record
    buffer_read(index, b_find);
    pin_minus();
    strcpy(ret_val, b_find.record);
    Lock_release(find);    // unlock the record
    return 0;
}
...
// all tables are closed using close_table function
int shutdown_db(){
    while(table_id > 0)
    {
...
void file_write_page(pagenum_t pagenum, const page_t src){
    ...
    if(key_check >= leaf_ORDER-fin_factor){   // bulk loading
